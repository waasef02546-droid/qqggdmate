...
| T7 owner-key rotation, approval, custody, and recovery | Not modeled by the current ProVerif files. | owner-local approval; owner registration version; wrap provenance; immutable rotation journal; object revision; canonical custody request digest; authenticated target wrapper | `presaga/crypto/key_custody.py`; `presaga/storage/encrypted_store.py`; `presaga/storage/mongo_encrypted_store.py`; `presaga/provider/registry.py`; `presaga/provider/server.py` | `tests/unit/test_key_custody.py`; `tests/security/test_key_custody_boundary.py`; `tests/integration/test_owner_key_rotation.py`; `test_rotation_recovery.py`; live `test_mongodb_rotation_cas.py` and `test_mongodb_provider_service.py`; semantic release custody gate | Prepare/export/owner-approval/custody/stage/commit or abort and cleanup recover across the tested JSON/Mongo states. Provider interfaces reject source private keys; unapproved target substitution, tampered/cross-object/conflicting artifacts fail before mutation, and exact retry is idempotent. The approval file and repository are trusted local inputs; restart restores the previously verified wrapper/digest without re-verifying the signature. The owner/KMS still handles the source key and DEK; no HSM, multi-object transaction, or production key-management guarantee is claimed. |
