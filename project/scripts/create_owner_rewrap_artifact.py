...
def create_artifact(request_path: Path, private_key_path: Path) -> dict[str, object]:
    ...
    return UmbralOwnerKeyCustody().rewrap(request, private_key).to_payload()
...
def main() -> int:
    ...
    artifact = create_artifact(args.request, args.source_private_key_file)
